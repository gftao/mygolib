...
func (w *fileLogWriter) doRotate(logTime time.Time) error {
	...
	if w.MaxLines > 0 || w.MaxSize > 0 {
		for ; err == nil && num <= 9999; num++ {
			fName = w.fileNameOnly + fmt.Sprintf(".%s.%04d%s", logTime.Format("2006-01-02"), num, w.suffix)
		...
		_, err = os.Lstat(fName)
		for ; err == nil && num <= 9999; num++ {
			fName = w.fileNameOnly + fmt.Sprintf(".%s.%04d%s", w.dailyOpenTime.Format("2006-01-02"), num, w.suffix)
			_, err = os.Lstat(fName)
		}
	}
